<template>
  <div class="candidates">
    ...
    <BasicTable
      ...
      @view-open-request="handleViewOpenRequest"
    >
    ...
    <a-modal
      ...
      title="Ver Open Request"
      width="1200px"
      :footer="null"
    >
      <OpenRequestView :open-request="selectedOpenRequest" />
    </a-modal>

    <a-modal
      v-model:open="showDownloadModal"
      title="Seleccionar Modelo de CV"
      @ok="confirmDownload"
    ...
    >
      <a-radio-group v-model:value="downloadOption">
        ...
        >
          Modelo Word-Ketos versión Inglés</a-radio
        >
        <a-radio
          value="accenture"
          style="display: block; margin-bottom: 10px"
    ...
    >
      <FormFeedback
        ref="feedbackWizardRef"
        v-if="selectedFeedbackRecord"
        :record="selectedFeedbackRecord"
        :totalSteps="feedbackItems.length"
        :initialStep="currentFeedbackIndex + 1"
        v-model="feedbackDraft"
        :loading="savingStep"
        @step-change="onFeedbackStepChange"
        @finish="saveFeedbackWizard"
        @next-request="handleNextRequest"
        @prev-request="handlePrevRequest"
      >
        <template
          v-for="(step, idx) in feedbackItems"
          :key="step.code"
          v-slot:['step-'.concat(idx+1)]="{ record, feedback, setFeedbackField }"
        >
          <div style="display: flex; flex-direction: column; gap: 12px">
            <a-alert
              :message="`Paso ${idx + 1}: ${step.title}`"
              type="info"
              show-icon
            />
            ...
            <div>
              <label style="font-weight: 600"
                >Marcar este paso como completado</label
              >
              <a-switch
                :checked="!!feedback[step.code]"
                @change="(checked) => setFeedbackField(step.code, checked)"
              />
...
import OpenRequestView from './components/view/OpenRequestView.vue';
...
const selectedOpenRequest = ref(null);
...
function getStatusColor(record) {
  const item = feedbackItems.value.find(
    (i) => i.title === record.current_status_label
  );
...
async function getSearch() {
  try {
    ...
    if (data && data.length > 0) {
      ...
      let subprofileName = '';
      if (profile) {
        const profileData = await fetch('get', 'primary-profiles', {
          id: profile
        });
      ...
      if (subprofile) {
        const subprofileData = await fetch('get', 'sub-profiles', {
          id: subprofile
        });
      ...
      searchTitle.value =
        code + ' - ' + client + ' - ' + profileName + ' - ' + subprofileName;
    }
  } catch (error) {}
}
  ...
  try {
    ...
    const castValue = (value, castConfig) => {
      ...
      if (!storedData || storedData === 'undefined') {
        console.warn(
          `No data found in localStorage for cast_${castConfig.source}`
        );
      ...
      try {
        ...
        console.error(
          `Error parsing localStorage data for cast_${castConfig.source}:`,
          error
        );
    ...
    candidates.value = result.map((item) => {
      ...
      if (feedbackItems.value[stateIndex]) {
        ...
        stateIndex >= feedbackItems.value.length &&
        feedbackItems.value.length > 0
      ) {
        // All completed
        newItem.current_status_label =
          feedbackItems.value[feedbackItems.value.length - 1].title;
      } else {
...
function handleViewOpenRequest(record) {
  selectedOpenRequest.value = record;
...
async function handleDownloadCV(record) {
  try {
    ...
    const candidateResp = await fetch('get', 'candidates/', {
  ...
  try {
    ...
    if (downloadOption.value === 'original') {
      ...
      if (!file?.s3_url) {
        showDownloadModal.value = false;
        message.warning(
          'El CV original activo no tiene URL de descarga (s3_url).'
        );
        return;
      }
      ...
      const nameFromKey =
        file.s3_key?.split('/').pop()?.split('?')[0] ||
        `cv-original-${candidateId}`;
    ...
    const formatted = await fetch('get', 'formatted-cvs/', {
      candidate: candidateId
    });
