  ...
  <div class="filters">
    <a-form layout="horizontal" ref="formRef" :model="filterInputs">
      <a-row :gutter="24">
        ...
        <a-row :gutter="24">
          <a-col :span="12">
            <a-form-item label="Criterio" name="name">
...
import { getCriterial, addCriterial, updateCriterial, deleteCriterial } from '@/api/criterial/criterial.js';
export default {
  name: 'criterialList',
  ...
  setup() {
    ...
    const fetchData = async (params = {}) => {
      try {
        const response = await getCriterial(params);
    ...
    const save = key => {
      ...
      if (data.id > 0) {
        ...
        updateCriterial(data.id, params).then(() => {
          fetchData();
        });
      } else {
        const { id, ...dataWithoutId } = data;
        addCriterial(dataWithoutId).then(() => {
          fetchData();
        });
      }
    };
