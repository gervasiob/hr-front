...
export async function exportToCsv(model, params = {}) {
    try {
        const response = await apiRequest('get', `reports/${model}/`, params, null, true);

        if (!response || !(response instanceof Blob)) {
            throw new Error('Respuesta inválida al exportar CSV');
        }
